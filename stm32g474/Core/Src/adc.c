...
uint16_t adc1_dma_buffer[ADC_BUFFER_SIZE];  // ADC1 �� DMA ��������
uint16_t adc3_dma_buffer[ADC_BUFFER_SIZE];  // ADC3 �� DMA ��������
/* USER CODE END 0 */
...
void HAL_ADC_MspDeInit(ADC_HandleTypeDef* adcHandle)
{
  ...
  /* USER CODE END ADC3_MspDeInit 0 */
    ...
    */
    ...
    HAL_GPIO_DeInit(GPIOE, GPIO_PIN_7|GPIO_PIN_9|GPIO_PIN_13);
...
if (hadc1.State == HAL_ADC_STATE_READY) {
        if (HAL_ADC_Start_DMA(&hadc1, (uint32_t*)adc1_dma_buffer, channel_num) != HAL_OK) {

        }
    }

    if (hadc3.State == HAL_ADC_STATE_READY) {
        if (HAL_ADC_Start_DMA(&hadc3, (uint32_t*)adc3_dma_buffer, channel_num) != HAL_OK) {
